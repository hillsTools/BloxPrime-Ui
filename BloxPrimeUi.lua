...
function sections:slider(props)
    ...
    local function slide(input)
        ...
        local size = math.clamp(pos - slider.color.AbsolutePosition.X, 0, slider.color.AbsoluteSize.X)
    ...
    sliderbutton.InputChanged:Connect(function(input)
        if slider.holding and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
            slide(input)
        end
    end)
    --
